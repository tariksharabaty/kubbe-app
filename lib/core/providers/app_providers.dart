...
final todayPrayerTimesProvider =
    AsyncNotifierProvider<TodayPrayerTimesNotifier, PrayerTimes?>(
        TodayPrayerTimesNotifier.new);

// TR: Bugünkü vakit notifier'ı
// EN: Today's prayer times notifier
class TodayPrayerTimesNotifier extends AsyncNotifier<PrayerTimes?> {
  @override
...
final nextPrayerProvider = Provider<String?>((ref) {
  ...
  return prayerTimesAsync.maybeWhen(
    data: (prayerTimes) {
      if (prayerTimes != null) {
        final nextPrayer = PrayerService.getNextPrayerTime(prayerTimes);
        return PrayerService.getPrayerNameTurkish(nextPrayer);
      }
      return null;
    },
    orElse: () => null,
...
final currentPrayerProvider = Provider<String?>((ref) {
  ...
  return prayerTimesAsync.maybeWhen(
    data: (prayerTimes) {
      if (prayerTimes != null) {
        final currentPrayer = PrayerService.getCurrentPrayer(prayerTimes);
        return PrayerService.getPrayerNameTurkish(currentPrayer);
      }
      return null;
    },
    orElse: () => null,
...
final qiblaDirectionProvider = FutureProvider<double>((ref) async {
  ...
  return PrayerService.getQiblaDirection(
    position.latitude,
    position.longitude,
  );
});
...
final lastKnownPositionProvider = FutureProvider<Position?>((ref) async {
  return await LocationService.getLastKnownPosition();
});
...
final locationQualityProvider = Provider<LocationQuality>((ref) {
  ...
  return positionAsync.maybeWhen(
    data: (position) => position != null
        ? LocationService.getLocationQuality(position.accuracy)
...
final isWithinTurkeyProvider = Provider<bool>((ref) {
  ...
  return positionAsync.maybeWhen(
    data: (position) => position != null
        ? LocationService.isWithinTurkey(position.latitude, position.longitude)
        : false,
    orElse: () => false,
  );
});
