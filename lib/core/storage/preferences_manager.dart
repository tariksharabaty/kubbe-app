...
class PreferencesManager extends Notifier<PreferencesState> {
  ...
  Future<void> setPrayerTimes(Map<String, dynamic> prayerTimesData) async {
    if (_prefs == null) return;

    await _prefs!.setString('prayerTimes', prayerTimesData.toString());
    await _prefs!
        .setString('lastPrayerUpdate', DateTime.now().toIso8601String());
  }

  // TR: Kaydedilmiş namaz vakitlerini al
  // EN: Get saved prayer times
  String? getPrayerTimes() {
    return _prefs?.getString('prayerTimes');
  }

  // TR: Son namaz güncelleme zamanını al
  // EN: Get last prayer update time
  String? getLastPrayerUpdate() {
    return _prefs?.getString('lastPrayerUpdate');
  }

  // TR: Konum verisini kaydet
  // EN: Save location data
  Future<void> setLocationData(Map<String, dynamic> locationData) async {
    if (_prefs == null) return;

    await _prefs!.setString('locationData', locationData.toString());
    await _prefs!
        .setString('lastLocationUpdate', DateTime.now().toIso8601String());
  }

  // TR: Kaydedilmiş konum verisini al
  // EN: Get saved location data
  String? getLocationData() {
    return _prefs?.getString('locationData');
  }

  // TR: Son konum güncelleme zamanını al
  // EN: Get last location update time
  String? getLastLocationUpdate() {
    return _prefs?.getString('lastLocationUpdate');
  }
}
